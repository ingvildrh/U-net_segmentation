...
'''
Save the predicted binary mask from the model to a folder
INPUT:
...
'''
Perform a bitwise and between the original image and the predicted binary mask to remove the background
INPUT:
    original_image : original image
    mask_image : predicted binary mask
    name : name of the image
'''
...
'''
Count the number of body pixels in the predicted binary mask
INPUT: 
    mask_image : predicted binary mask
OUTPUT:
    count : number of body pixels
'''
def count_body_pixels(mask_image):
    return np.count_nonzero(mask_image), np.size(mask_image)

'''
Write the number of body pixels to a .JSON file
INPUT:
    name : name of the image
    count : number of body pixels
...
'''
Create the output for the body model by removing the background and put the images in a folder
Saves the number of body pixels to a .JSON file
...
'''
Converts a greyscale mask to a RGB mask
'''
def mask_parse(mask):
